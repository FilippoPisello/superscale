...
NUMBER_UOM = rf"{NUMBER}\s*(<uom-regex>)(?:$|\s)"  # 30g | 30 g | 30.0 g
NO_SYMBOL_NUMBER_UOM = rf"(?:^|\s|[a-z]|[a-z].){NUMBER_UOM}"  # NOT 15/30kg | A/30kg
FRACTION_UOM = rf"{FRACTION}\s*(<uom-regex>)(?:$|\s)"
NUMBER_UOM_LETTER = rf"{NUMBER}\s*(<uom-regex>)(?:$|\s|[a-z])"  # 30gA | 30 gA | 30.0 gA


NUMBERx = r"(\d+)\s*x\s*"  # 30x | 30 x
xNUMBER = (
    r"(?<!\d)x\s*(\d+)(?:$|\s|[a-z])"  # 'x30' | 'x 30' | NOT '10x30' | NOT 'x30.3'
)

xNUMBER_LETTER = (
    r"(?<!\d)x\s*(\d+)(?:$|\s)"  # 'x30' | 'x 30' | NOT '10x30' | NOT 'x30.3'
)
